<template>
  <div>
        ...
        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="consultoria in consultoriasPaginadas" :key="consultoria.conr_id">
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-900 dark:text-gray-300">{{ formatEmptyColumn(consultoria.Trámite, 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-900 dark:text-gray-300">{{ formatEmptyColumn(consultoria.Dependencia, 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-900 dark:text-gray-300">{{ formatEmptyColumn(consultoria['Empresa cliente'], 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatEmptyColumn(consultoria['Fecha de registro'], 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatEmptyColumn(consultoria['Fecha de despacho'], 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatEmptyColumn(consultoria.Asunto, 10) }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <a
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatEmptyColumn(consultoria.Observacion, 10) }}</div>
            </td>
            <td v-if="mostrarBotones" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
              <div class="relative">
                <button :id="`dropdown-button-${consultoria.conr_id}`" @click="toggleDropdown(consultoria.conr_id)" class="bg-white border border-gray-300 rounded-md p-2 flex items-center">
                  Seleccionar acción
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 9l6 6 6-6" />
                  </svg>
                </button>
                <ul :id="`dropdown-menu-${consultoria.conr_id}`" v-if="dropdownStates[consultoria.conr_id]" class="absolute z-10 bg-white border border-gray-300 rounded-md mt-1 w-full">
                  <li @click="handleActionChange('editar', consultoria)" class="flex items-center p-2 hover:bg-gray-100 cursor-pointer">
                    <i class="fa fa-pencil-alt mr-2"></i>
                    Editar
                  </li>
                  <li @click="handleActionChange('eliminar', consultoria)" class="flex items-center p-2 hover:bg-gray-100 cursor-pointer">
                    <i class="fa fa-trash mr-2"></i>
                    Eliminar
                  </li>
                  <li @click="handleActionChange('detalles', consultoria)" class="flex items-center p-2 hover:bg-gray-100 cursor-pointer">
    ...
    <div class="mt-4 flex justify-end gap-2" v-if="hasRole('exportar_excel')">
      <button @click="exportarTodasConsultorias" class="bg-green-600 hover:bg-green-800 text-white font-bold py-2 px-4 rounded flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          <path d="M3 3a2 2 0 012-2h10a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V3zm2 0v14h10V3H5zm3 4h4v2H8V7zm0 4h4v2H8v-2z" />
        </svg>
        Exportar BD
      </button>
      <button @click="exportarVistaActual" class="bg-blue-600 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          <path d="M3 3a2 2 0 012-2h10a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V3zm2 0v14h10V3H5zm3 4h4v2H8V7zm0 4h4v2H8v-2z" />
        </svg>
    ...
    <div class="mt-6 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 sm:px-6">
      <div class="flex flex-1 justify-between items-center">
        ...
        <div class="flex gap-2">
          <button @click="$emit('cambiar-pagina', currentPage - 1)" :disabled="currentPage === 1" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-300 shadow-sm">
          ...
          <button @click="$emit('cambiar-pagina', currentPage + 1)" :disabled="currentPage === totalPages" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-300 shadow-sm">
            Siguiente
...
const { exportarTodasConsultorias } = useConsultoria();
...
const cabecerasVisibles = computed(() => {
  return mostrarBotones.value ? props.cabecerasTabla : props.cabecerasTabla.slice(0, -1);
});
...
const exportarVistaActual = () => {
  ...
  XLSX.writeFile(wb, 'consultorias_segmentada.xlsx');
};

const acortarTexto = (texto, maxLength) => {
  return texto.length > maxLength ? texto.substring(0, maxLength) + '...' : texto;
};

const acortarTexto2 = (texto, maxLength) => {
  return texto.length > maxLength ? texto.substring(0, maxLength) + '' : texto;
};
