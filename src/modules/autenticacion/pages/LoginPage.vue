...
const submit = async () => {
  try {
    ...
    if (response) {
      ...
      // Obtener el privilegio de las rutas
      const privilegio = rutas.some(ruta => ruta.roru_privilegio === 1) ? 1 : 0;

      // Actualizar el store con los datos del login, incluyendo el privilegio
      store.onLogginSuccess(true, usuario.usu_nombres, rol.rol_id, "Login exitoso", rutas, token, privilegio); // Pasar el privilegio como argumento
